...
void physical_rc(){
  if (Serial.available()) {
  String data = Serial.readStringUntil('\n');
  if (data.startsWith("PHYSICAL_REMOTE")) {
      int parsed_values[9];

      int parsed_count = sscanf(data.c_str(), "PHYSICAL_REMOTE,%d,%d,%d,%d,%d,%d,%d,%d", &parsed_values[0], &parsed_values[1], &parsed_values[2], &parsed_values[3], &parsed_values[4], &parsed_values[5], &parsed_values[6], &parsed_values[7]);
