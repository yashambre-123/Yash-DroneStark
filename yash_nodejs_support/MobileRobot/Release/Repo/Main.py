...
def main():
    ...
    try:
        ...
        while True:   
            ...
            cv2.imshow("Original frame", np.array(frame, dtype = np.uint8 ))
            # cv2.waitKey()
            if cv2.waitKey(1) == ord('c'):
                break
            #checkBlue, imgBlue = checkBlueColor(frame)
            checkBlue, imgBlue = checkBlueColor(frame)
            # cv2.imshow("Blue color", imgBlue)
            
            # print("CheckBlue: ", checkBlue)
            if(checkBlue == 1):
                # print("CheckBlue: ", checkBlue)  
                stop_no_line()                                
                ...
                if decodedData == "":
                    ...
                    # decodedData = decodedData.replace('b','1',1)
                    decodedData = decodedData[1]
                    print(int(decodedData))
                    ...
                    # cv2.imshow("grayscale",im)imgYellow
                    #maskFrame = imgBlue
                    checkBlue = 0
            else:
                # print("Detecting yellow line")
                # print("CheckBlue: ", checkBlue)
