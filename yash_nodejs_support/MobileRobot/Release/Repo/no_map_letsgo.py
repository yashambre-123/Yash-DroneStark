...
def Go():
    ...
    is_moving = 0
    #tgt_node = node_list_proxy[proxy_cnt]
    prev_node = -1
    ...
    while True:
        ...
        if(current_state == "L"):            
            ...
            if(decodeCount == 0):
                prev_node = cur_node                
                direction, cur_node = decodeQR(qr_data, tgt_node, prev_node)
                #prev_node = cur_node
                if(1):
                ...
                if(proxy_cnt < len(node_list_proxy)-1):
                #if(proxy_cnt < len(motionPath)-1):
                    proxy_cnt += 1
                    tgt_node = node_list_proxy[proxy_cnt]
                    #tgt_node = motionPath[proxy_cnt]
                else:
                    tgt_node = prev_node
                #prev_node = cur_node
            decodeCount += 1
            print("Current node: %s"%cur_node)
            print("Target node: %s"%tgt_node)
            ...
            print("Start and end node are as follows:")
